...
PVAState TangentialFrameMechanization::update(const IMUMeasurement& imu, 
    ...
    Eigen::Matrix3d Ohm_t__tb = C_t__b_in * Ohm_t__ie * C_t__b_in.transpose() - Ohm_t__ie;
    ...
    Eigen::Matrix3d C_bm__b_bar = Eigen::Matrix3d::Identity()
                                + (1 - cos(alpha_cross_mag)) * alpha_cross / (alpha_cross_mag * alpha_cross_mag)
                                + (alpha_cross * alpha_cross) / (alpha_cross_mag * alpha_cross_mag) 
                                * (1-(sin(alpha_cross_mag))/(alpha_cross_mag));

    Eigen::Matrix3d C_t__b_bar = C_t__b_in * C_bm__b_bar - 0.5 * Ohm_t__tb * C_t__b_in * dt;
    ...
    Eigen::Vector3d a_t__tb = f_t__ib + g_t__b - 2 * Ohm_i__ie * v_t__tb_in;
